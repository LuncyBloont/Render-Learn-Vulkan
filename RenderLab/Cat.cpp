...
void Cat::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage,
    ...
    if (vkCreateBuffer(core->device, &bufferCreateInfo, nullptr, &buffer)
        != VK_SUCCESS) {
        throw std::runtime_error("Failed to create buffer.");
    }

    VkMemoryRequirements memRequest{};
    vkGetBufferMemoryRequirements(core->device, buffer, &memRequest);

    VkMemoryAllocateInfo allocateInfo{};
    allocateInfo.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
    allocateInfo.allocationSize = memRequest.size;
    allocateInfo.memoryTypeIndex = findMemoryType(memRequest.memoryTypeBits, properties);

    if (vkAllocateMemory(core->device, &allocateInfo, nullptr, &bufferMemory)
        != VK_SUCCESS) {
...
uint32_t Cat::getSuitableDevice(VkPhysicalDevice device, uint32_t* minScore) {
    ...
    if (indices.graphicsIndice.has_value() &&
        ...
        indices.transferIndice.has_value()) {
        addScore();
    }
